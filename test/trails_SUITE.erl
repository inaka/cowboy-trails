...
basic_compile_test(_Config) ->
    BasicRoute =
        [{"localhost", [{"/such/path", http_such_path_handler, []}, {"/very", http_very, []}]}],
...
static_trails3_constructor(_Config) ->
    StaticRoute = [{'_', [trails:trail("/", cowboy_static, {private_file, "index.html"})]}],
...
static_trails4_constructor(_Config) ->
    StaticRouteTrails =
        [{'_', [trails:trail("/", cowboy_static, {private_file, "index.html"}, #{}, [])]}],
    StaticRouteCowboy = get_static_route(),
...
basic_metadata(_Config) ->
    ...
    Trail = trails:trail("/", cowboy_static, {private_file, "index1.html"}, Metadata, []),
...
put_metadata(_Config) ->
    ...
    Trail = trails:trail("/", cowboy_static, {private_file, "index2.html"}, Metadata, []),
    Metadata = trails:metadata(Trail),
    {comment, ""}.
...
post_metadata(_Config) ->
    ...
    Trail = trails:trail("/", cowboy_static, {private_file, "index3.html"}, Metadata, []),
    Metadata = trails:metadata(Trail),
    {comment, ""}.
